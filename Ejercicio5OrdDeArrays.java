package ejercicio5orddearrays;
import java.util.Scanner;
import java.util.Arrays;

public class Ejercicio5OrdDeArrays {
    ...
    public static void main(String[] args) {
        ...
        int[] copiaBurbuja = arr5.clone();
        burbuja(copiaBurbuja);

        System.out.println("Burbuja:");
        mostrarArray(copiaBurbuja);
        contarNumeros(copiaBurbuja);


        int[] copiaSeleccion = arr5.clone();
        seleccion(copiaSeleccion);

        System.out.println("Seleccion:");
        mostrarArray(copiaSeleccion);
        contarNumeros(copiaSeleccion);


        int[] copiaInsercion = arr5.clone();
        insercion(copiaInsercion);

        System.out.println("Insercion:");
        mostrarArray(copiaInsercion);
        contarNumeros(copiaInsercion);


        int[] copiaArrays = arr5.clone();
        Arrays.sort(copiaArrays);

        System.out.println("Arrays.sort:");
        mostrarArray(copiaArrays);
        contarNumeros(copiaArrays);


        int[] copiaQuick = arr5.clone();
        quickSort(copiaQuick, 0,
        copiaQuick.length - 1);

        System.out.println("QuickSort:");
        mostrarArray(copiaQuick);
        contarNumeros(copiaQuick);


        int[] copiaShell = arr5.clone();
        shell(copiaShell);

        System.out.println("Shell:");
        mostrarArray(copiaShell);
        contarNumeros(copiaShell);


        int[] copiaMerge = arr5.clone();
        mergeSort(copiaMerge, 0,
        copiaMerge.length - 1);

        System.out.println("MergeSort:");
        mostrarArray(copiaMerge);
        contarNumeros(copiaMerge);
